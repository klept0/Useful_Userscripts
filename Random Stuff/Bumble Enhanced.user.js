...
// @namespace   https://greasyfork.org/scripts/440988-bumble-enhanced/code/Bumble%20Enhanced.user.js
// @match       https://*.bumble.com/*
// @version     2.6.5
// @author      habs - mods by reddit.com/user/AM_NOT_BANANA_AMA and klept0
...
function makeSVG(className, onlineStatus) {
  const svg1 = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  svg1.setAttribute("height", 80);
  svg1.setAttribute("width", 80);
  svg1.classList.add(className);
  svg1.style.position = "absolute";
  svg1.style.zIndex = "9";
  svg1.style.opacity = ".9";
  ...
  const fill = onlineStatus === 1 ? 'green' : onlineStatus === 2 ? 'yellow' : onlineStatus === 3 ? 'grey' : 'grey';
...
window.setInterval(() => {
  try {
    ...
    carousel.forEach(car => {
      ...
      if (!match) return; 
      const div = makeSVG ('showBumbleCarouselOnline', match.online_status);
  ...
  try {
    ...
    msgs.forEach(msg => {
      ...
      if (!match) return;
      const div = makeSVG ('showBumbleMsgsOnline', match.online_status);
      msg.prepend(div);
    });
